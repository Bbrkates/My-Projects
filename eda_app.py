...
def run_eda_app():
    ...
    if submenu == "Descriptive":
        ...
        with col1:
            with st.expander("Dist Plot of Diabetes"):
                fig = plt.figure()
                # sns.countplot(df['Diabetes']) # Bu yorumlanmış, kullanılabilir.
                # st.pyplot(fig)
                
                gen_df = df['Outcome'].value_counts().to_frame()
                gen_df = gen_df.reset_index()
                gen_df.columns = ['Diabetes Type', 'Counts']
                # st.dataframe(gen_df) # Opsiyonel olarak tabloyu gösterebilirsiniz.
    ...
    #         st.dataframe(df['Glucose'].value_counts())
        ...
        with st.expander("Frequency Dist Plot of Age"):
            # Eski Matplotlib kodu (opsiyonel olarak kullanılabilir)
            # fig, ax = plt.subplots()
